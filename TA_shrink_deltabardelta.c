...
double oldmain(double **pointset, int n, int d)
{
  ...
  { // Initialization
    ...
    for (i = 1; i <= outerloop; i++)
    {
      ...
      update_points(&fxc, &current, xc_index, xn_pm_index, xn_extraminus_index, k, mc);
    ...
    for (i = 1; i <= outerloop; i++)
    {
      ...
      for (p = 1; p <= innerloop; p++)
      {
        ...
        // Get random neighbor
        generate_neighbor_deltabardelta(xc_index, xn_pm_index, xn_extraminus_index, k, mc);
